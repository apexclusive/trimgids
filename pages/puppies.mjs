...
export function puppiesPage(list = []) {
  ...
  <article class="pm-card" data-id="${esc(p.id)}" data-breed="${esc(p.breedSlug)}" data-province="${esc(p.province)}" data-city="${esc(p.city)}" data-breeder="${esc(p.breeder)}" data-price="${esc(String(p.price))}" data-title="${esc(p.title)}" tabindex="0" role="button" aria-label="Bekijk ${esc(p.title)}">
    <div class="pm-media">
      ...
      <span class="pm-badge">${p.isDemo ? 'DEMO · voorbeeldadvertentie' : 'Gecontroleerd profiel'}</span>
...
<div class="pm-grid" id="pm-grid">${itemsHtml}<a class="pm-ad-cta" href="#pm-form"><span class="pm-ad-cta-badge">Gratis t/m 31 december</span><h3>Jouw nest hier?</h3><p>Plaats nu je echte, controleerbare nest gratis. Voeg gezondheidschecks en je contactgegevens toe; na controle wordt je advertentie zichtbaar.</p><strong>Plaats je gratis advertentie →</strong></a></div>
...
<div class="pm-dialog" id="pm-dialog" role="dialog" aria-modal="true" aria-labelledby="pm-dialog-title" hidden>
  ...
  <div class="pm-dialog-card">
    ...
    <div class="pm-dialog-body">
      <span class="eyebrow">Demo · voorbeeldweergave</span>
      ...
      <div class="pm-dialog-cta">
        <a class="btn" id="pm-dialog-contact" href="#">Plaats straks jouw echte advertentie →</a>
...
(function () {
  ...
  function openPup(id) {
    ...
    dContact.href = p.isDemo ? '#pm-form' : 'mailto:' + encodeURIComponent(p.email || '') + '?subject=' + encodeURIComponent('Interesse in ' + p.title + ' (' + p.id + ')') + '&body=' + encodeURIComponent('Beste ' + p.breeder + '. Ik ben geïnteresseerd in dit nest en wil graag een kennismaking plannen. Met vriendelijke groet,' );
    dContact.textContent = p.isDemo ? 'Plaats straks jouw echte advertentie →' : 'Mail de fokker over dit nest →';
    dialog.hidden = false;
