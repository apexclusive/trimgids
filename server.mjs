...
function mapPage() {
  return `<!doctype html><html lang="nl"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Interactieve Landkaart Honden Nederland: Salons, Scholen & Bossen | TrimGids</title><meta name="description" content="Interactieve kaart met alle trimsalons, hondenscholen, hondenhotels, hydrotherapie en losloopgebieden in Nederland. Direct route plannen via Google Maps."><link rel="canonical" href="https://trimgids.nl/kaart"><meta name="robots" content="index, follow, max-image-preview:large, max-snippet:-1, max-video-preview:-1"><link rel="stylesheet" href="https://unpkg.com/leaflet@1.9.4/dist/leaflet.css" integrity="sha256-p4NxAoJBhIIN+hmNHrzRCf9tD/miZyoHS5obTRR9BMY=" crossorigin=""><link rel="preconnect" href="https://fonts.googleapis.com"><link href="https://fonts.googleapis.com/css2?family=Fraunces:opsz,wght@9..144,600;9..144,700&family=Inter:wght@400;500;600&display=swap" rel="stylesheet"><style>${directoryStyles()}${customModuleStyles()}#map-container{height:680px;width:100%;border-radius:24px;border:1px solid var(--line);box-shadow:0 4px 20px rgba(0,0,0,.06);margin-top:20px;z-index:1}.popup-card h4{font-family:Fraunces,serif;font-size:18px;margin:4px 0 6px}.popup-card p{color:var(--muted);margin:4px 0 8px;font-size:13px}.popup-card .p-meta{display:flex;gap:8px;font-size:12px;margin-bottom:8px;flex-wrap:wrap}.popup-card .btn-nav-map{display:inline-block;background:var(--green);color:#fff!important;padding:8px 14px;border-radius:999px;font-weight:700;font-size:12px;text-decoration:none;margin-top:6px}</style></head><body><header><nav><a class="logo" href="/">🐾 TrimGids</a><div class="nav-links"><a href="/trimsalon">Trimsalons</a><a href="/kaart" style="color:var(--green);font-weight:700">Interactieve Kaart</a><a href="/verzekering">Hondenverzekering</a><a href="/wandelen">Wandelen</a><a href="/nieuws">Nieuws</a><a href="/vermist">Vermist</a><a href="/hondenbelasting">Hondenbelasting</a><a href="/">Home</a></div></nav></header><main><p class="crumb"><a href="/">TrimGids</a> / Interactieve Landkaart</p><span class="eyebrow">Navigatie & Locatiezoeker</span><h1>Interactieve Landkaart voor Honden in Nederland</h1><p class="intro">Vind en vergelijk direct alle gecertificeerde trimsalons, hondenscholen, hondenhotels en losloopbossen bij jou in de buurt. Klik op een markering voor tarieven en directe navigatie via Google Maps of Apple Maps.</p><div class="map-controls" style="display:flex;gap:12px;flex-wrap:wrap;align-items:center"><input type="search" id="map-search" placeholder="🔍 Zoek salon, school of plaats..." style="flex:1;min-width:240px;padding:12px 16px;border:1px solid var(--line);border-radius:12px;font:inherit"><button id="btn-geoloc" class="btn" style="background:#fff;border:1px solid var(--line);padding:11px 18px;border-radius:12px;font-weight:600;cursor:pointer">📍 Toon mijn locatie</button></div><div class="filter-bar" style="margin:16px 0 0"><button class="f-btn active" data-filter="all">Alles tonen</button><button class="f-btn" data-filter="trimsalon">✂️ Trimsalons</button><button class="f-btn" data-filter="hondenschool">🎓 Hondenscholen</button><button class="f-btn" data-filter="opvang">🏨 Opvang & Hotels</button><button class="f-btn" data-filter="wellness">💆 Wellness & Fysio</button><button class="f-btn" data-filter="routes">🌲 Wandelen & Stranden</button></div><div id="map-container"></div><section class="next"><span class="eyebrow">Bekijk ook</span><h2>Ontdek meer diensten</h2><div class="next-links"><a href="/trimsalon/pomeriaan">Trimsalon Pomeriaan →</a><a href="/trimsalon/labradoodle">Trimsalon Labradoodle →</a><a href="/trimsalon/maastricht">Trimsalons in Maastricht →</a><a href="/verzekering">Hondenverzekering Vergelijken →</a><a href="/wandelen">Wandelroutes in Limburg →</a><a href="/hondenbelasting">Hondenbelasting Tarieven →</a></div></section></main><footer><a href="/">TrimGids</a><span>Navigatiegegevens worden direct doorgezet naar Google Maps / Apple Maps.</span></footer><script src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js" integrity="sha256-20nQCchB9co0qIjJZRGuk2/Z9VM+kNiyxNV1lvTlZBo=" crossorigin=""></script><script>let map;let markers=[];let activeFilter='all';let allLocations=[];const initMap=()=>{map=L.map('map-container').setView([51.5,5.6],8);L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{attribution:'&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors',maxZoom:19}).addTo(map);loadData();};const loadData=async()=>{try{const [provRes,routesRes]=await Promise.all([fetch('/api/providers'),fetch('/api/routes')]);const provData=await provRes.json();const routesData=await routesRes.json();allLocations=[...(provData.providers||[]).map(p=>({...p,isRoute:false})),...(routesData.routes||[]).map(r=>({...r,isRoute:true,name:r.title,category:'routes'}))];renderMarkers();}catch(e){console.error('Data error:',e);}};const renderMarkers=()=>{markers.forEach(m=>map.removeLayer(m));markers=[];const query=document.getElementById('map-search').value.toLowerCase().trim();const filtered=allLocations.filter(item=>{const matchCat=activeFilter==='all'||(item.category===activeFilter)||(activeFilter==='routes'&&item.isRoute);const matchQ=!query||(item.name&&item.name.toLowerCase().includes(query))||(item.city&&item.city.toLowerCase().includes(query))||(item.province&&item.province.toLowerCase().includes(query));return matchCat&&matchQ;});const bounds=[];filtered.forEach(item=>{if(!item.lat||!item.lng)return;const mapsUri='https://www.google.com/maps/dir/?api=1&destination='+encodeURIComponent(item.lat+','+item.lng)+'&travelmode=driving';const marker=L.marker([item.lat,item.lng]);const popupHtml='<div class="popup-card"><h4>'+(item.isRoute?'🌲 ':'🐾 ')+(item.name||'Locatie')+'</h4><div class="p-meta"><span>📍 '+(item.city||'')+(item.province?' ('+item.province+')':'')+'</span>'+(item.rating?'<span>⭐ '+item.rating+' ('+item.reviewCount+')</span>':'')+(item.startingPrice?'<span>💶 Vanaf €'+item.startingPrice+'</span>':'')+'</div><p>'+(item.description||item.address||'')+'</p><a class="btn-nav-map" href="'+mapsUri+'" target="_blank" rel="noopener noreferrer">🧭 Navigeer met Google Maps ↗</a></div>';marker.bindPopup(popupHtml);marker.addTo(map);markers.push(marker);bounds.push([item.lat,item.lng]);});if(bounds.length){map.fitBounds(bounds,{padding:[40,40],maxZoom:14});}};document.querySelectorAll('.f-btn').forEach(btn=>btn.addEventListener('click',()=>{document.querySelectorAll('.f-btn').forEach(b=>b.classList.remove('active'));btn.classList.add('active');activeFilter=btn.dataset.filter;renderMarkers();}));document.getElementById('map-search').addEventListener('input',renderMarkers);document.getElementById('btn-geoloc').addEventListener('click',()=>{if(navigator.geolocation){navigator.geolocation.getCurrentPosition(pos=>{const lat=pos.coords.latitude;const lng=pos.coords.longitude;map.setView([lat,lng],13);L.circleMarker([lat,lng],{radius:8,fillColor:'#2f6b4f',color:'#fff',weight:3,opacity:1,fillOpacity:0.9}).addTo(map).bindPopup('<strong>📍 Jouw huidige locatie</strong>').openPopup();});}else{alert('Geolocatie wordt niet ondersteund door je browser.');}});initMap();</script></body></html>`;
}
