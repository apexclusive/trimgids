...
import { dogpediaPage } from './pages/dogpedia.mjs';
import { steunPage, supporterCreate } from './pages/steun.mjs';
...
/* Ronde 18 — een last-minute-plek is alleen "live" tot aan de begintijd van de
   plek zelf (of tot het eind van de dag als er geen tijd bekend is).
   Verlopen (demo-)slots blijven in de data staan, maar worden nergens meer als
   actuele deal getoond: niet op de homepage, niet op /last-minute, niet via de API. */
function isLastMinuteSlotLive(slot, ref = new Date()) {
  if (!slot || slot.claimed) return false;
  const day = String(slot.date || '').slice(0, 10);
  if (!/^\d{4}-\d{2}-\d{2}$/.test(day)) return false;
  const endOfDay = new Date(day + 'T23:59:59');
  if (isNaN(endOfDay)) return false;
  const tm = String(slot.time || '').match(/(\d{1,2}):(\d{2})/);
  if (tm) {
    const start = new Date(day + 'T00:00:00');
    start.setHours(Number(tm[1]), Number(tm[2]), 0, 0);
    return !isNaN(start) && start >= ref;
  }
  return endOfDay >= ref;
}
...
async function reviewCreate(slug, input) {
  ...
  const body = clean(input.body, 1000);
  const city = clean(input.city, 60);
  const visited = input.visited === true || input.visited === 'true' || input.visited === 'on';
  if (!author || !validEmail(email) || !Number.isInteger(rating) || rating < 0 || rating > 10 || body.length < 25 || !visited) throw new Error('review_invalid_fields');
  const all = await collectionList(reviewsFile);
  if (all.some(r => r.providerSlug === slug && r.email === email && (Date.now() - Date.parse(r.createdAt || 0)) < 180 * 86400000)) throw new Error('review_already_given');
  return collectionAdd(reviewsFile, { id: randomUUID(), providerSlug: clean(slug, 100), author, email, rating, body, city, visited, status: 'pending', createdAt: new Date().toISOString() });
}

/* Baasjescijfer: gemiddelde van goedgekeurde reviews (0-10), pas zichtbaar
   vanaf 3 beoordelingen — dat houdt een eerste enthousiaste of ontevreden
   stem eerlijk in perspectief. */
const reviewsScoreCache = new Map();
function baasjesScore(slug) {
  const hit = reviewsScoreCache.get(slug);
  if (hit && Date.now() - hit.at < 60000) return hit.data;
  const all = loadJsonLocal(reviewsFile) || [];
  const list = all.filter(r => r.providerSlug === slug && r.status === 'approved' && Number.isFinite(Number(r.rating)));
  const data = list.length ? {
    score: Math.round((list.reduce((sum, r) => sum + Number(r.rating), 0) / list.length) * 10) / 10,
    count: list.length,
    visible: list.length >= 3
  } : null;
  reviewsScoreCache.set(slug, { at: Date.now(), data });
  return data;
}
/* Baasjescijfer: intern 0-10 (10 niveaus), weergave als vertrouwde 5-sterren-schaal
   met halve sterren — zoals Google en marktplaatsen. Het cijfer kan nooit gekocht
   of verhoogd worden door een bedrijf. */
function starRow(score5) {
  const pct = Math.max(0, Math.min(100, Math.round((score5 / 5) * 100)));
  return '<span class="stars" role="img" aria-label="' + score5.toFixed(1).replace('.', ',') + ' van 5 sterren"><span class="stars-bg" aria-hidden="true">★★★★★</span><span class="stars-fg" aria-hidden="true" style="width:' + pct + '%">★★★★★</span></span>';
}
function baasjesBadge(slug) {
  const b = baasjesScore(slug);
  if (!b) return '';
  if (!b.visible) return '<span class="pc-baasjes pc-baasjes-new">Baasjescijfer · ' + b.count + '/3 beoordelingen</span>';
  const five = (b.score / 2).toFixed(1).replace('.', ',');
  return '<span class="pc-baasjes">' + starRow(b.score / 2) + '<strong>' + five + '</strong> Baasjescijfer · ' + b.count + (b.count === 1 ? ' beoordeling' : ' beoordelingen') + '</span>';
}
...
function generateSitemapUncached(now) {
  ...
  add('/dogpedia', '0.9', 'weekly');
  add('/steun', '0.7', 'monthly');
...
function dnaPage() {
  return `<!doctype html><html lang="nl"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Beste Honden DNA Test 2026: Embark vs Wisdom Panel Vergelijken | TrimGids</title><meta name="description" content="Vergelijk de beste honden DNA- & gezondheidstesten van 2026 (Embark, Wisdom Panel, Orivet). Ontdek de raszuiverheid, stamboom en 250+ erfelijke aandoeningen."><link rel="canonical" href="https://trimgids.nl/dna-test"><meta name="robots" content="index, follow, max-image-preview:large, max-snippet:-1, max-video-preview:-1"><link rel="stylesheet" href="/assets/css/fonts.css?v=1" id="tg-local-fonts"><script type="application/ld+json">${(() => { const dd = loadJsonLocal(dnaTestsFile); const tests = (dd && (Array.isArray(dd) ? dd : dd.tests)) || []; return JSON.stringify({ "@context": "https://schema.org", "@graph": [{ "@type": "ItemList", name: "Honden DNA-testen vergeleken 2026", itemListElement: tests.map((t, i) => ({ "@type": "ListItem", position: i + 1, name: t.title, url: "https://trimgids.nl/dna-test#dna-" + t.id })) }, ...tests.map(t => ({ "@type": "Product", name: t.title, description: t.description, brand: { "@type": "Brand", name: t.provider }, aggregateRating: { "@type": "AggregateRating", ratingValue: t.rating, reviewCount: t.reviewCount }, offers: { "@type": "Offer", priceCurrency: "EUR", price: t.salePrice, priceValidUntil: "2026-12-31", availability: "https://schema.org/InStock", url: t.affiliateUrl } }))] }); })()}</script>
...
function providerPage(pathname) {
  ...
  return `<!doctype html><html lang="nl"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>${escapeHtml(title)}</title><meta name="description" content="${escapeHtml(description)}"><link rel="canonical" href="https://trimgids.nl${canonical}"><meta name="robots" content="index, follow, max-image-preview:large, max-snippet:-1, max-video-preview:-1"><meta property="og:type" content="business.business"><meta property="og:title" content="${escapeHtml(title)}"><meta property="og:description" content="${escapeHtml(description)}"><meta property="og:url" content="https://trimgids.nl${canonical}"><link rel="stylesheet" href="/assets/css/fonts.css?v=1" id="tg-local-fonts"><script type="application/ld+json">${schemaJson}</script><style>${directoryStyles()}.form-row{display:grid;gap:8px;margin-top:18px}.form-row label{display:grid;gap:5px;color:var(--muted);font-size:14px}.form-row input,.form-row select,.form-row textarea{font:inherit;border:1px solid var(--line);border-radius:10px;padding:10px}.form-row textarea{min-height:100px;resize:vertical}.form-row button{justify-self:start}.review-score-card{display:flex;align-items:center;gap:16px;background:linear-gradient(135deg,#0f3e28,#17694a);color:#fff;border-radius:16px;padding:18px 22px;margin:6px 0 4px}.review-score-card strong{font-size:16px}.review-score-card p{margin:2px 0 0;font-size:13px;color:rgba(255,255,255,.8)}.score-big{font:800 40px Fraunces,Georgia,serif;line-height:1;color:#6ee7b7;background:rgba(255,255,255,.12);border:1px solid rgba(255,255,255,.22);border-radius:14px;padding:10px 16px}.rating-label{font-weight:700;color:var(--green)}.review-check{display:flex!important;flex-direction:row!important;align-items:flex-start;gap:10px;font-size:13px;color:var(--ink-2)}input[type=range]{accent-color:var(--green);width:100%}.star-input{display:inline-flex;gap:3px;-webkit-user-select:none;user-select:none}.star-btn{background:none;border:0;cursor:pointer;font-size:26px;line-height:1;color:#d8dee4;padding:2px 3px;transition:transform .16s var(--ease),color .16s}.star-btn:hover{transform:scale(1.18)}.star-btn.on{color:#f59e0b}.star-input:hover .star-btn{color:#f1b95c}.review-head{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:2px}.review-stars .stars{width:84px;height:15px}.review-stars .stars-bg,.review-stars .stars-fg{font-size:15px}.review-how{background:var(--card);border:1px solid var(--line);border-radius:14px;padding:10px 16px;margin:10px 0 4px}.review-how summary{cursor:pointer;font-weight:700;font-size:14px;color:var(--ink)}.review-how p{color:var(--muted);font-size:13px;line-height:1.55;margin:8px 0}</style></head><body><header><nav><a class="logo" href="/">🐾 TrimGids</a><div class="nav-links"><a href="/trimsalon/${citySlug}/${breedSlug}">Terug naar ${breed.name} ${place.name}</a><a href="/kaart">Kaart</a><a href="/verzekering">Hondenverzekering</a><a href="/wandelen">Wandelen</a><a href="/nieuws">Nieuws</a><a href="/vermist">Vermist</a><a href="/hondenbelasting">Hondenbelasting</a></div></nav></header><main><p class="crumb"><a href="/">TrimGids</a> / <a href="/trimsalon/${citySlug}">${escapeHtml(place.name)}</a> / <a href="/trimsalon/${citySlug}/${breedSlug}">${escapeHtml(breed.name)}</a> / ${escapeHtml(provider.name)}</p><span class="eyebrow">${provider.demo ? 'Voorbeeldprofiel' : 'Geverifieerd Aanbiederprofiel'}</span><h1>${escapeHtml(provider.name)}</h1><p class="intro">${escapeHtml(description)}</p><div class="summary"><strong>€${provider.startingPrice || 55}</strong><span>vanafprijsindicatie<br><small>Inclusief wassen, drogen en rasverzorging</small></span></div><section><div class="section-head"><div><span class="eyebrow">Aanbiedersdetails</span><h2>Waarom deze salon bij jouw ${breed.name} past</h2></div><a class="outline" href="${maps}" target="_blank" rel="noopener noreferrer">Kaart & route →</a></div><article class="provider"><div><span class="label">Geverifieerde TrimGids-partner</span><h2>${escapeHtml(provider.name)}</h2><p class="address">${escapeHtml(provider.address)}</p><div class="chips">${provider.specializations.map(item => `<span>${escapeHtml(item)}</span>`).join('')}</div></div><div class="provider-actions"><a href="${maps}" target="_blank" rel="noopener noreferrer">🧭 Google Maps openen →</a>${provider.phone ? `<a href="tel:${escapeHtml(provider.phone)}" style="color:var(--green);font-weight:700">📞 ${escapeHtml(provider.phone)}</a>` : ''}<a href="/claim?slug=${encodeURIComponent(provider.slug)}&name=${encodeURIComponent(provider.name)}&city=${encodeURIComponent(provider.city)}&addr=${encodeURIComponent(provider.address)}" style="font-size:12px;color:var(--green);font-weight:700">🏢 Bent u de eigenaar? Claim dit gratis profiel →</a></div></article></section><section class="guide"><h2>Vachtverzorging voor een ${escapeHtml(breed.name)}</h2><p>${escapeHtml(breed.summary)}</p><ul>${breed.care.map(item => `<li>${escapeHtml(item)}</li>`).join('')}</ul></section><section class="guide"><h2>Baasjesbeoordelingen & Ervaringen</h2><div id="own-reviews"><p>Reviews laden...</p></div><div class="review-score-card" id="review-score-card"><p>Reviews laden...</p></div><details class="review-how"><summary>Hoe werkt het Baasjescijfer? (en hoe blijft het eerlijk?)</summary><p><strong>1 · Eén eerlijke check:</strong> je geeft je e-mailadres en vinkt aan dat je er écht bent geweest. Daarmee valideert TrimGids dat beoordelingen van echte baasjes komen.</p><p><strong>2 · Gemiddelde vanaf 3:</strong> het gemiddelde cijfer wordt pas openbaar getoond als er minstens <strong>3 goedgekeurde beoordelingen</strong> zijn — zo voorkomt één boze (of juist opgetogen) reactie een misleidend beeld.</p><p><strong>3 · Moderatie:</strong> elke beoordeling wordt nagekeken op taalgebruik, relevantie en uitvoerbaarheid. Uit de hand gelopen of misleidende teksten worden niet gepubliceerd. Bedrijven kunnen hun cijfer niet kopen of verhogen.</p><p><strong>4 · Maximaal 1 per 180 dagen:</strong> per e-mailadres en locatie kan maximaal één beoordeling per half jaar worden geplaatst. Zo blijft het cijfer een eerlijk beeld met de tijd.</p><p><strong>5 · Anoniem publiceren:</strong> je e-mail wordt nooit getoond; op de pagina zie je alleen je voornaam, woonplaats en cijfer.</p></details><form id="review-form" class="form-row">
<label>Jouw naam *<input name="author" required maxlength="40" placeholder="Voor- of roepnaam"></label>
<label>E-mailadres *<input name="email" type="email" required maxlength="120" placeholder="alleen voor verificatie & moderatie, nooit zichtbaar"></label>
<label>Woonplaats (optioneel)<input name="city" maxlength="60" placeholder="Bijv. Utrecht"></label>
<label>Het Baasjescijfer — hoe was het écht? *<span class="star-input" id="star-input" role="radiogroup" aria-label="Baasjescijfer in sterren"><button type="button" class="star-btn" data-star="1" aria-label="1 ster — Slecht">★</button><button type="button" class="star-btn" data-star="2" aria-label="2 sterren — Matig">★</button><button type="button" class="star-btn" data-star="3" aria-label="3 sterren — Voldoende">★</button><button type="button" class="star-btn" data-star="4" aria-label="4 sterren — Goed">★</button><button type="button" class="star-btn" data-star="5" aria-label="5 sterren — Uitstekend">★</button></span><span id="rating-label" class="rating-label">4 — Goed</span><input type="hidden" name="rating" id="rating-input" value="8" required></label>
<label class="review-check"><input type="checkbox" name="visited" required> Ik ben hier écht geweest met mijn hond (echte ervaring, geen gerucht)</label>
<label>Je ervaring *<textarea name="body" required minlength="25" maxlength="1000" placeholder="Minimaal 25 tekens: hoe reageerde je hond, was het resultaat naar wens, hoe was de communicatie?"></textarea></label>
<button class="outline" type="submit">Baasjesbeoordeling insturen</button>
<p id="review-status"></p></form></section></main><footer>
  <div style="width:100%;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:16px;margin-bottom:18px">
    <a class="logo" href="/" style="font-size:20px">🐾 TrimGids</a>
    <div style="display:flex;gap:12px;font-size:13px;font-weight:600;flex-wrap:wrap">
      <a href="/trimsalon">Trimsalons</a>
...
</script><script>const providerSlug=${JSON.stringify(provider.slug)};const reviews=document.getElementById('own-reviews');const scoreCard=document.getElementById('review-score-card');const status=document.getElementById('review-status');const ratingInput=document.getElementById('rating-input');const starInput=document.getElementById('star-input');const starButtons=starInput?Array.from(starInput.querySelectorAll('.star-btn')):[];const labels=['','Slecht','Matig','Voldoende','Goed','Uitstekend'];const starsHtml=pct=>'<span class="stars" aria-hidden="true"><span class="stars-bg">★★★★★</span><span class="stars-fg" style="width:'+pct+'%">★★★★★</span></span>';const ratingLabel=document.getElementById('rating-label');const applyStars=star=>{starButtons.forEach((b,i)=>{b.classList.toggle('on',i<star);b.setAttribute('aria-checked',String(i<star));});if(ratingInput)ratingInput.value=star*2;if(ratingLabel)ratingLabel.textContent=star+' — '+labels[star];};starButtons.forEach(b=>{b.addEventListener('click',()=>applyStars(Number(b.getAttribute('data-star'))));b.setAttribute('aria-checked','false');});applyStars(4);const renderScore=summary=>{if(!scoreCard)return;if(!summary){scoreCard.innerHTML='<strong>Baasjescijfer: nog geen beoordelingen</strong><p>Wees de eerste baas die een eerlijk cijfer geeft — vanaf 3 beoordelingen wordt het cijfer openbaar getoond.</p>';return;}if(!summary.visible){scoreCard.innerHTML='<strong>Baasjescijfer in opbouw</strong><p>Gemiddelde nu: <strong>'+summary.score.toLocaleString('nl-NL')+'/10</strong> ('+summary.count+'/'+summary.count+' beoordelingen in wachtrij). Nog '+(3-summary.count)+' beoordeling(en) nodig voordat het cijfer openbaar wordt.</p>';return;}const five=summary.score/2;scoreCard.innerHTML='<div class="score-big">'+five.toFixed(1).replace('.',',')+'</div><div><strong>Baasjescijfer '+starsHtml(Math.min(100,Math.round((five/5)*100)))+'</strong><p>Gemiddelde van '+summary.count+' goedgekeurde baasjesbeoordelingen ('+summary.score.toLocaleString('nl-NL')+'/10 intern bewaard).</p></div>';};const render=items=>{reviews.replaceChildren();if(!items.length){reviews.append(Object.assign(document.createElement('p'),{textContent:'Nog geen goedgekeurde Baasjesbeoordelingen. Wees de eerste!'}));return;}items.forEach(item=>{const article=document.createElement('article');article.className='google-review';const head=document.createElement('div');head.className='review-head';const title=document.createElement('strong');title.textContent=item.author+(item.city?' ('+item.city+')':'');const arr=document.createElement('span');arr.className='review-stars';arr.innerHTML=starsHtml(Math.min(100,Math.round(((item.rating/2)/5)*100)));head.append(title,arr);const body=document.createElement('p');body.textContent=item.body;article.append(head,body);reviews.append(article);});};fetch('/api/providers/'+encodeURIComponent(providerSlug)+'/reviews').then(response=>response.json()).then(data=>{renderScore(data.summary);render(data.reviews||[]);}).catch(()=>{reviews.textContent='Beoordelingen zijn tijdelijk niet beschikbaar.';});document.getElementById('review-form').addEventListener('submit',async event=>{event.preventDefault();const data=new FormData(event.currentTarget);const response=await fetch('/api/providers/'+encodeURIComponent(providerSlug)+'/reviews',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({author:data.get('author'),email:data.get('email'),city:data.get('city'),rating:Number(data.get('rating')),body:data.get('body'),visited:!!data.get('visited')})});if(response.ok){status.textContent='Bedankt! Je Baasjesbeoordeling staat klaar voor moderatie en telt daarna mee voor het Baasjescijfer.';event.currentTarget.reset();applyStars(4);}else{const err=await response.json().catch(()=>({}));status.textContent=err.error==='review_already_given'?'Je hebt deze locatie al beoordeeld (max. 1 per 6 maanden).':'Je beoordeling kon niet worden verstuurd. Controleer je gegevens (cijfer 0-10, min. 25 tekens, bezoek-vinkje).';}});</script></body></html>`;
}
...
function directoryPage(pathname, pageParam = 1) {
  ...
  <section class="tip-box" style="margin-top:45px">
    ...
    <form class="form-grid" id="dir-quote-form">
      ...
      <p id="dir-quote-status" role="status" aria-live="polite" class="status-msg full" style="display:none"></p>
    </form>
  </section>`;
...
<main>
  ...
  <div class="dir-toolbar" role="search" aria-label="Aanbieders zoeken, filteren en sorteren">
    <div class="dir-search">
      ...
      <input id="dir-q" type="search" enterkeyhint="search" placeholder="Zoek op naam, plaats of specialisatie…" autocomplete="off" aria-label="Zoek aanbieders">
    </div>
...
<section class="dir-explore-sec" aria-label="${isFallback ? 'Aanbieders in jouw regio' : 'Aanbieders vinden en ontdekken'}">
    ...
    <div class="dir-explore is-on" id="pane-explore" role="tabpanel" aria-labelledby="tab-explore">
      ...
      <div class="dir-mini-list" aria-label="Aanbieders in de omgeving">
        <div class="dir-hero">
          <img src="${(PROVIDER_IMG[category] || PROVIDER_IMG.trimsalon).src}" srcset="${(PROVIDER_IMG[category] || PROVIDER_IMG.trimsalon).srcset}" sizes="(max-width:700px) 100vw, 50vw" width="960" height="420" loading="lazy" decoding="async" alt="${(PROVIDER_IMG[category] || PROVIDER_IMG.trimsalon).alt}">
...
function lastMinutePage() {
  ...
  /* Ronde 18 — verlopen plekken (zoals de oude demo-slots) worden niet meer
     als actueel getoond; de pagina toont dan één duidelijk demo-voorbeeld. */
  const liveSlots = allSlots.filter(s => isLastMinuteSlotLive(s));
...
function modernizeGeneratedHtmlUncached(html) {
  ...
  html = html
    ...
    .replace('</head>', '<link rel="icon" href="/favicon.svg?v=4" type="image/svg+xml"><link rel="manifest" href="/manifest.webmanifest"><link rel="stylesheet" href="/assets/css/site-polish.css?v=1" id="tg-site-polish">' + routeSkin + '</head>')
  ...
  if (!html.includes('theme-color')) {
    html = html.replace('<meta charset="UTF-8">', '<meta charset="UTF-8"><meta name="theme-color" content="#ffffff" media="(prefers-color-scheme: light)"><meta name="theme-color" content="#0e1512" media="(prefers-color-scheme: dark)">');
  }
...
/* Theme vóór de eerste paint. De fallback is bewust dezelfde als in de
  ...
  if (html.includes('data-nl-map') && !html.includes('tg-nlmap-js')) {
    ...
    const mapTags = (needsCss ? '<link rel="stylesheet" href="/assets/css/nl-map.css?v=8">' : '')
      + (needsLeaflet ? '<link rel="preconnect" href="https://basemaps.cartocdn.com" crossorigin><link rel="stylesheet" href="/assets/vendor/leaflet/leaflet.css"><script id="tg-leaflet-js" src="/assets/vendor/leaflet/leaflet.js" defer></script>' : '')
      + '<script id="tg-nlmap-js" src="/assets/js/nl-map.js?v=17" defer></script>';
...
<form action="/zoek" method="get" role="search"><input type="search" enterkeyhint="search" name="q" value="${escapeHtml(query)}" placeholder="Zoek bijvoorbeeld: hondenbelasting, hitteberoerte, verzekering…" aria-label="Zoek in TrimGids" autofocus><button type="submit">Zoeken</button></form>
...
export async function handleRequest(req, res) {
  ...
  try {
    ...
    if (url.pathname === '/api/home' && req.method === 'GET') {
      ...
      return publicJson(res, 200, {
        ...
        lastMinute: { slots: (lm.length ? lm : lastMinuteData.slots).filter(s => isLastMinuteSlotLive(s)).slice(0, 3) },
    ...
    if (url.pathname === '/api/last-minute' && req.method === 'GET') {
      ...
      let list = (slots.length ? slots : lastMinuteData.slots).filter(s => isLastMinuteSlotLive(s));
    ...
    /* Supporters API */
    if (url.pathname === '/api/supporters' && req.method === 'POST') {
      if (!rateLimit(req, rateLimits.write, 5, 60000)) return json(res, 429, { error: 'rate_limited' });
      return json(res, 201, { supporter: await supporterCreate(await readJson(req)) });
    }
    ...
    if (url.pathname.startsWith('/api/providers/') && url.pathname.endsWith('/reviews') && req.method === 'GET') {
      const slug = decodeURIComponent(url.pathname.slice('/api/providers/'.length, -'/reviews'.length));
      const list = (await collectionList(reviewsFile)).filter(r => r.providerSlug === slug && r.status === 'approved').map(({ email, ...publicFields }) => publicFields);
      return json(res, 200, { reviews: list, summary: baasjesScore(slug) });
    }
    ...
    /* Ronde 9 — zoekpagina */
    /* Ronde 30 — offline-fallbackpagina voor de service worker: als een
     pagina niet in de cache zit en er is geen netwerk, tonen we een
     vriendelijke uitleg i.p.v. de kale browserfout. */
  if (url.pathname === '/offline') {
    const offlineHtml = `<!doctype html><html lang="nl"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Offline — TrimGids</title><meta name="robots" content="noindex"><link rel="stylesheet" href="/assets/css/fonts.css?v=1" id="tg-local-fonts"><style>body{margin:0;font-family:'Plus Jakarta Sans',system-ui,sans-serif;background:#0e1512;color:#edf5f0;display:grid;place-items:center;min-height:100vh;text-align:center;padding:24px}main{max-width:420px}h1{font-size:26px;margin:14px 0 10px}p{color:#b4c3ba;line-height:1.7;font-size:15px}a{display:inline-block;margin-top:20px;padding:13px 26px;border-radius:999px;background:linear-gradient(135deg,#34d399,#10b981);color:#052e1d;font-weight:800;text-decoration:none}</style></head><body><main><div style="font-size:44px" aria-hidden="true">🐕</div><h1>Even geen verbinding</h1><p>Deze pagina staat niet in je offline cache. Zodra je weer internet hebt, laadt TrimGids gewoon door — je eerder bezochte pagina's blijven ondertussen gewoon werken.</p><a href="/">Opnieuw proberen</a></main></body></html>`;
  ...
  if (url.pathname === '/zoek' || url.pathname === '/search' || url.pathname === '/zoeken') {
    ...
    if (url.pathname === '/dogpedia' || url.pathname === '/hondenpedia' || url.pathname === '/hondengids' || url.pathname === '/hondenrecords') {
      res.writeHead(200, secureHeaders({ 'Content-Type': 'text/html; charset=utf-8', 'Cache-Control': HTML_CACHE }));
      return res.end(dogpediaPage());
    }
    if (url.pathname === '/steun' || url.pathname === '/doneer' || url.pathname === '/steun-trimgids') {
      res.writeHead(200, secureHeaders({ 'Content-Type': 'text/html; charset=utf-8', 'Cache-Control': HTML_CACHE }));
