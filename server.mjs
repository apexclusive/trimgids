...
function baasjesBadge(slug) {
  ...
  return '<span class="pc-baasjes"><strong>' + b.score.toFixed(1).replace('.', ',') + '</strong> Baasjescijfer · ' + b.count + (b.count === 1 ? ' beoordeling' : ' beoordelingen') + '</span>';
}
...
<label>Het Baasjescijfer (0-10) *<input name="rating" id="rating-slider" type="range" min="0" max="10" step="1" value="8" required><span id="rating-label" class="rating-label">8 — Goed</span></label>
...
</script><script>const providerSlug=${JSON.stringify(provider.slug)};const reviews=document.getElementById('own-reviews');const scoreCard=document.getElementById('review-score-card');const scoreLabel=document.getElementById('rating-label');const scoreSlider=document.getElementById('rating-slider');const status=document.getElementById('review-status');const scoreText=r=>r+((r>=9)?' — Uitstekend':(r>=8)?' — Goed':(r>=7)?' — Ruim voldoende':(r>=6)?' — Voldoende':(r>=5)?' — Matig':(r>=3)?' — Zwak':' — Slecht');const renderScore=summary=>{if(!scoreCard)return;if(!summary){scoreCard.innerHTML='<strong>Baasjescijfer: nog geen beoordelingen</strong><p>Wees de eerste baas die een eerlijk cijfer geeft — vanaf 3 beoordelingen wordt het cijfer openbaar getoond.</p>';return;}if(!summary.visible){scoreCard.innerHTML='<strong>Baasjescijfer in opbouw</strong><p>Gemiddelde nu: <strong>'+summary.score.toLocaleString('nl-NL')+'</strong> ('+summary.count+'/'+summary.count+' beoordelingen in wachtrij). Nog '+(3-summary.count)+' beoordeling(en) nodig voordat het cijfer openbaar wordt.</p>';return;}scoreCard.innerHTML='<div class="score-big">'+summary.score.toLocaleString('nl-NL')+'</div><div><strong>Baasjescijfer</strong><p>Gemiddelde van '+summary.count+' goedgekeurde baasjesbeoordelingen (0-10).</p></div>';};const render=items=>{reviews.replaceChildren();if(!items.length){reviews.append(Object.assign(document.createElement('p'),{textContent:'Nog geen goedgekeurde Baasjesbeoordelingen. Wees de eerste!'}));return;}items.forEach(item=>{const article=document.createElement('article');article.className='google-review';const title=document.createElement('strong');title.textContent=item.rating+'/10 · '+item.author+(item.city?' ('+item.city+')':'');const body=document.createElement('p');body.textContent=item.body;article.append(title,body);reviews.append(article);});};fetch('/api/providers/'+encodeURIComponent(providerSlug)+'/reviews').then(response=>response.json()).then(data=>{renderScore(data.summary);render(data.reviews||[]);}).catch(()=>{reviews.textContent='Beoordelingen zijn tijdelijk niet beschikbaar.';});if(scoreSlider){const refresh=()=>{scoreLabel.textContent=scoreSlider.value+' '+scoreText(Number(scoreSlider.value));};scoreSlider.addEventListener('input',refresh);refresh();}document.getElementById('review-form').addEventListener('submit',async event=>{event.preventDefault();const data=new FormData(event.currentTarget);const response=await fetch('/api/providers/'+encodeURIComponent(providerSlug)+'/reviews',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({author:data.get('author'),email:data.get('email'),city:data.get('city'),rating:Number(data.get('rating')),body:data.get('body'),visited:!!data.get('visited')})});if(response.ok){status.textContent='Bedankt! Je Baasjesbeoordeling staat klaar voor moderatie en telt daarna mee voor het Baasjescijfer.';event.currentTarget.reset();scoreLabel.textContent=scoreSlider.value+' '+scoreText(Number(scoreSlider.value));}else{const err=await response.json().catch(()=>({}));status.textContent=err.error==='review_already_given'?'Je hebt deze locatie al beoordeeld (max. 1 per 6 maanden).':'Je beoordeling kon niet worden verstuurd. Controleer je gegevens (cijfer 0-10, min. 25 tekens, bezoek-vinkje).';}});</script></body></html>`;
}
